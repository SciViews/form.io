...
> 'form.io' allows for numbering and captioning of figures, tables and equations (scientific notation) for many different R Markdown documents. LaTeX as weel as the 'bookdown' R package provide these features, but they are limited to a set of output formats. The solution implemented in 'form.io' works in many places, including the default HTML, PDF and Word export formats, as well as, for R Notebooks. **To experiment with it, download the present file from [here](), open it in RStudio, and look at the various exportation formetas under the _knit_ button.** 
...
- The legend of tables and figures do not look like legends at all in the `.Rmd` file, and this is against the phylosophy of Markdown.
...
With `label` being the identifier to use in `\@ref(fig:label)` or `\@ref(tab:label)` to reference it. Of course the final caption mut be reworked to include a number, according to the table/figure position in the document, and the same number must be reused in the link produced by `\@ref()`.
...
Mixing online R expressions with R chunks works everywhere, except in R Notebook, because R inline expressions are *not* run in the editor! So, it is much better to place an `Table()` code (with only the caption text as argument) *inside* a chunk. If you don't want to display that line of code (because you want just demonstrate nice and clean R code and after all, the table caption is part of the narration, not the code), you can put it in a distinct chunk, or better, you can put it in the chunk in first position and use `echo=2:n` where `n` is greated or equal to the number of R statements in the chunk.
...
Of course, `Figure()` is compatible with `knitr::include_graphics()` too (**But you cannot use Markdown formattings currently in the legend for LaTeX outputs, and there are other strange things, for instance, if I don't escape the underscorte, it does not work on LaTeX**):
...
However, if you insist on using a more conventional approach by using `fig.cap=` instead, the new mechanims is still compatible with it... just remember to embed your caption text in `fig()`, which gives `fig.cap=fig("My caption text")`. Here is an example:
...
Mathjax, the engine used to manage R Markdown and Pandoc equations is able to handle this for HTML and PDF outputs, but not for Word. So, we want an alternate solution that also displays nicely in RStudio. Here it is. See in RStudio how our own implementation previews the equation with the tag set at right and with the equation label in it. This is nice to differenciate numbered and non numbered equations, and also to spot the equation label for future reference. You just have to add `\-> <-tag{eq:label}` at the end of your display equation, just before the final `$$`.
...
- Equations numberirng in the 'bookdown' version of PDF seems to be buggy.
...
- It is currently obligatory to indicate `fig_caption:yes` explicitly in HTLML and Word documents to have the figure captions actually rendered (we plan another solution).
